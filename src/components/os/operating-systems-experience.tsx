...
const CTX = "Foundation · Operating Systems @ ShadowX Financial Corporation. Instruction is enterprise SOC-flavoured, referencing Windows AD, Linux servers, and SIEM telemetry.";
...
export function OperatingSystemsExperience() {
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 md:px-8">
      ...
      <Section icon={Cpu} eyebrow="Section 3" title="Operating System Explorer" subtitle="Every concept an OS gives you — click to inspect.">
        <ConceptExplorer />
      </Section>

      <Section icon={Cpu} eyebrow="Section 4" title="Interactive Process Explorer" subtitle="Live view of an analyst's Windows 11 endpoint.">
        <ProcessExplorer />
      </Section>

      <Section icon={HardDrive} eyebrow="Section 5" title="Enterprise Filesystem" subtitle="Browse the filesystem shape of Windows, Linux, and macOS.">
        <FilesystemBrowser />
      </Section>

      <Section icon={MemoryStick} eyebrow="Section 6" title="Memory Visualizer" subtitle="How RAM is carved up per process — and where attackers hide.">
        <MemoryVisualizer />
      </Section>

      <Section icon={Power} eyebrow="Section 7" title="Boot Process Simulator" subtitle="From power-on to desktop, step-by-step.">
        <BootSimulator />
      </Section>

      <Section icon={Wrench} eyebrow="Section 8" title="Service Manager" subtitle="Start / stop / disable services and see the business impact.">
        <ServiceManager />
      </Section>

      <Section icon={AlertTriangle} eyebrow="Section 9" title="Enterprise Incident" subtitle="Live SIEM alert. Investigate and recommend action.">
        <IncidentInvestigation />
      </Section>

      <Section icon={FlaskConical} eyebrow="Section 10" title="Operating System Labs" subtitle="Ten hands-on missions modelled on real ShadowX tickets.">
        <LabsGrid />
      </Section>

      <Section icon={Sparkles} eyebrow="Section 11" title="AI Mentor" subtitle="Ask anything about the OS. Get an answer scoped to a SOC L1, sysadmin, or interview.">
        <MentorPanel />
      </Section>

      <Section icon={Network} eyebrow="Section 12" title="Knowledge Graph" subtitle="How every OS concept connects. Hover a node.">
        <KnowledgeGraph />
      </Section>

      <Section icon={Briefcase} eyebrow="Section 13" title="Career Mapping" subtitle="Where operating-system knowledge shows up in eight roles you might grow into.">
        <CareerMap />
      </Section>
    </div>
  );
  ...
  return (
    <div className="glass-panel space-y-4 rounded-2xl p-6">
      ...
      <p className="text-sm leading-relaxed text-muted-foreground">
        You've just badged in as a SOC Analyst at {COMPANY.name} — {COMPANY.employees.toLocaleString()} employees,
        {" "}{COMPANY.countries} countries, {COMPANY.branches} branch offices, {COMPANY.units.length} business units.
        Every alert you'll investigate this year originates on an operating system:
        a Windows domain controller, a Linux trading server, an analyst's laptop, or a cloud node.
      </p>
      <div className="grid gap-3 md:grid-cols-3">
        {[
          { t: "Why OS matters", d: "The OS is the security boundary attackers cross. Understanding it separates a triager from a hunter." },
          { t: "Where alerts come from", d: "Sysmon on Windows, auditd on Linux, EDR on endpoints — every telemetry stream is an OS-level signal." },
          { t: "What SOC does", d: "Read process trees, correlate events, decide contain-or-monitor. Nothing here is theoretical." },
        ].map((c) => (
...
function ListField({ label, items, mono, tone }: { label: string; items: string[]; mono?: boolean; tone?: "danger" }) {
  return (
    <div className="rounded-lg border border-border/60 bg-background/40 p-3">
      <div className="text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground">{label}</div>
...
function ConceptExplorer() {
  ...
  return (
    <div className="glass-panel grid gap-4 rounded-2xl p-4 md:grid-cols-[220px_1fr] md:p-6">
      ...
      <div key={sel.id} className="animate-fade-in space-y-3 rounded-lg border border-border/60 bg-background/30 p-5">
        <div className="text-xs font-medium uppercase tracking-[0.15em] text-cyber">{sel.title}</div>
...
function ProcessExplorer() {
  ...
  const view = useMemo(() => {
    let r = rows.filter((p) =>
      [p.name, String(p.pid), p.user, p.signed].some((s) => s.toLowerCase().includes(q.toLowerCase()))
    );
    r = [...r].sort((a, b) => (sort === "pid" ? a.pid - b.pid : (b as any)[sort] - (a as any)[sort]));
    return r;
  }, [rows, q, sort]);
  ...
  return (
    <div className="glass-panel space-y-3 rounded-2xl p-4 md:p-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
        ...
        <Button variant="outline" size="sm" onClick={() => setRows(PROCESSES)}>Reset</Button>
      </div>
          ...
          <tbody>
            {view.map((p) => (
              ...
              >
                ...
                <td className="p-2 font-mono">{p.name}{p.suspicious && <span className="ml-1 text-destructive">⚠</span>}</td>
                ...
                <td className="p-2 text-right">
                  <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); terminate(p.pid); }}>
                    <X className="h-3 w-3" />
                  </Button>
                </td>
              </tr>
      ...
      <Dialog open={!!sel} onOpenChange={(o) => !o && setSel(null)}>
        <DialogContent>
          {sel && (
            <>
              <DialogHeader>
                <DialogTitle className="font-mono">{sel.name} · pid {sel.pid}</DialogTitle>
                <DialogDescription>
                  Parent PID {sel.ppid} · owned by {sel.user} · signed by {sel.signed}.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-2 text-sm">
                <div>CPU {sel.cpu}% · Mem {sel.mem} MB · {sel.status}</div>
                {sel.suspicious ? (
  ...
  return (
    <div className="glass-panel space-y-3 rounded-2xl p-4 md:p-6">
      <Tabs value={os.os} onValueChange={(v) => pick(FILESYSTEMS.find((f) => f.os === v)!)}>
        <TabsList>
          {FILESYSTEMS.map((f) => (
            <TabsTrigger key={f.os} value={f.os}>{f.os}</TabsTrigger>
          ))}
        </TabsList>
        {FILESYSTEMS.map((f) => (
          <TabsContent key={f.os} value={f.os} className="space-y-3">
            <div className="flex items-center gap-2 rounded-md border border-border/60 bg-background/40 px-3 py-2 font-mono text-xs">
              ...
              <span>{path}</span>
              {path !== f.root && (
                <Button size="sm" variant="ghost" className="ml-auto h-6" onClick={() => setPath(f.root)}>
                  ← root
                </Button>
              )}
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              {entries.map((e) => {
                const child = e.kind === "dir" ? `${path}${path.endsWith("/") || path.endsWith("\\") ? "" : os.os === "Windows" ? "\\" : "/"}${e.name}` : null;
...
function BootSimulator() {
  ...
  return (
    <div className="glass-panel space-y-4 rounded-2xl p-4 md:p-6">
        ...
        {BOOT_STEPS.map((s, i) => (
          <button
            ...
            className={cn(
              ...
              i <= step ? "border-cyber/60 bg-cyber/10 text-foreground" : "border-border/60 text-muted-foreground"
            )}
          >
      ...
      <div key={active.id} className="animate-fade-in rounded-lg border border-border/60 bg-background/40 p-4">
        <div className="text-sm font-semibold">{active.label}</div>
        ...
        <div className="mt-2 rounded-md border border-cyber/30 bg-cyber/5 p-2 text-xs">
          <span className="font-semibold text-cyber">ShadowX: </span>{active.enterprise}
        </div>
      </div>
    </div>
  );
        ...
        {rows.map((s) => (
          <div key={s.id} className="rounded-lg border border-border/60 bg-background/40 p-3">
            <div className="flex flex-col gap-2 md:flex-row md:items-center">
              ...
              <Badge
                variant="outline"
                className={cn(s.status === "Running" ? "border-emerald-500/50 text-emerald-500" : "border-muted text-muted-foreground")}
              >
                {s.status}
              </Badge>
              <Select value={s.startup} onValueChange={(v) => set(s.id, { startup: v as Service["startup"] })}>
              ...
              <div className="flex gap-1">
                <Button size="sm" variant="outline" onClick={() => set(s.id, { status: "Running" })}>
                  <Play className="h-3 w-3" />
                </Button>
                <Button size="sm" variant="outline" onClick={() => set(s.id, { status: "Stopped" })}>
            ...
            <div className="mt-2 rounded-md border border-cyber/30 bg-cyber/5 p-2 text-xs">
              <span className="font-semibold text-cyber">Impact: </span>{s.impact}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="glass-panel space-y-4 rounded-2xl p-4 md:p-6">
      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4">
        <div className="text-[10px] font-semibold uppercase tracking-[0.15em] text-destructive">SIEM Notable</div>
        <div className="mt-1 text-sm">{INCIDENT.alert}</div>
      </div>
      <div>
        <div className="mb-2 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Investigation facts</div>
      ...
      {done && (
        <div className="rounded-lg border border-cyber/40 bg-cyber/5 p-4">
          <div className="text-sm font-semibold">Score: {score} / {INCIDENT.questions.length}</div>
    ...
    <div className="glass-panel rounded-2xl p-4 md:p-6">
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {LABS.map((l) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] text-cyber">LAB {String(l.id).padStart(2, "0")}</span>
              {completed.has(l.id) && <Badge variant="outline" className="border-emerald-500/50 text-emerald-500">Done</Badge>}
            </div>
      ...
      <Dialog open={!!sel} onOpenChange={(o) => !o && setSel(null)}>
        <DialogContent className="max-w-3xl">
          {sel && (
            <>
              <DialogHeader>
                <DialogTitle>Lab {String(sel.id).padStart(2, "0")} · {sel.title}</DialogTitle>
              ...
              <ScrollArea className="max-h-[70vh]">
                <div className="space-y-4 pr-3 text-sm">
                  ...
                  <div>
                    <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">Assessment</div>
                    <div className="space-y-2">
                  ...
                  <Button
                    onClick={() => { setCompleted((s) => new Set(s).add(sel.id)); setSel(null); }}
                    className="w-full"
                  >
                    Mark lab complete
                  </Button>
...
function LabBlock({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">{title}</div>
      <div className="rounded-md border border-border/60 bg-background/40 p-3 text-xs">{children}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-md border border-border/60 bg-background/40 p-3 text-xs">
      ...
      {show ? (
        ...
        <Button size="sm" variant="ghost" className="mt-1 h-6 px-2 text-[10px]" onClick={() => setShow(true)}>
          Reveal answer
        </Button>
      )}
    </div>
    ...
    <div className="glass-panel rounded-2xl p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="text-sm text-muted-foreground">
          Ask about processes, kernel mode, PIDs, memory, filesystems — or say "explain as SOC L1" or "for interview".
        </div>
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          <div key={p} className="rounded-md border border-border/60 bg-background/40 p-2 text-xs text-muted-foreground">
            💬 {p}
          </div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isHover ? "border-cyber bg-cyber/20 text-cyber shadow-glow" :
                isConn ? "border-cyber/60 bg-cyber/10 text-foreground" :
