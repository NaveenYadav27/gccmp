    ...
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-8">
      <header className="glass-panel relative overflow-hidden rounded-2xl p-6 md:p-8">
        ...
        <div className="relative">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Every mission you run happens inside this fictional bank. Click any node to see how it
            lives in the enterprise.
          </p>
          <div className="mt-4 flex flex-wrap gap-3 text-xs">
            <span className="flex items-center gap-1.5 rounded-md border border-border/50 bg-surface-1 px-2 py-1">
              <MapPin className="h-3 w-3 text-cyber" />
              HQ {ORG.hq}
            </span>
            <span className="flex items-center gap-1.5 rounded-md border border-border/50 bg-surface-1 px-2 py-1">
              <Users className="h-3 w-3 text-cyber" />
              {ORG.employees.toLocaleString()} employees
            </span>
            <span className="flex items-center gap-1.5 rounded-md border border-border/50 bg-surface-1 px-2 py-1">
              <Building2 className="h-3 w-3 text-cyber" />
              {ORG.sites.length} sites
            </span>
            <span className="flex items-center gap-1.5 rounded-md border border-border/50 bg-surface-1 px-2 py-1">
              <Server className="h-3 w-3 text-cyber" />
              {NODES.length} core systems
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-4">
          <Card className="glass-panel p-4">
            <div className="mb-2 text-xs font-semibold uppercase tracking-[0.15em] text-cyber">
              Crown Jewels
            </div>
            ...
            <>
              ...
              <div className="mt-4 space-y-4 text-sm">
                ...
                {selected.concepts.length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold uppercase tracking-[0.15em] text-cyber">
                      Concepts
                    </div>
                ...
                {selected.relatedSlugs.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {selected.relatedSlugs.map((s) => (
                      ...
                      >
                        <Link to="/session/$slug" params={{ slug: s }}>
                          Open · {s}
                        </Link>
                      </Button>
                    ))}
...
function Field({
  ...
  return (
    <div
      className={`rounded-lg border p-3 ${tone === "danger" ? "border-danger/40 bg-danger/5" : tone === "cyber" ? "border-cyber/40 bg-cyber/5" : "border-border/50 bg-surface-1"}`}
    >
      <div className="text-[10px] font-semibold uppercase tracking-[0.15em] text-cyber">
        {label}
      </div>
