...
function NotesPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("session_notes").select("content").eq("session_slug", active).maybeSingle();
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-8 md:grid-cols-[240px_1fr] md:px-8">
      <aside className="glass-panel h-fit rounded-2xl p-3">
        <div className="mb-2 flex items-center gap-2 px-2 py-1">
          ...
          <span className="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Missions</span>
        </div>
        <div className="space-y-0.5">
          {MONTH_1.map((s) => (
            ...
            >
              <span className="mr-2 font-mono text-[10px] text-muted-foreground">M{String(s.number).padStart(2, "0")}</span>
              {s.title}
            </button>
          ))}
        </div>
      ...
      <div className="glass-panel rounded-2xl p-5">
        <div className="mb-3 flex items-center justify-between">
          <h1 className="text-lg font-bold">Notes · {MONTH_1.find((s) => s.slug === active)?.title}</h1>
