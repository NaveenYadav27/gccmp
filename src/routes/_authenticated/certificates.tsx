    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 md:px-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Earned on completion. Verifiable on your profile.
        </p>
      </header>
      <div className="grid gap-4 md:grid-cols-2">
        {CERTS.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className={`glass-panel rounded-2xl p-5 ${unlocked ? "border-cyber/40" : ""}`}
            >
              <div className="mb-3 flex items-start justify-between">
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl ${unlocked ? "cyber-gradient text-cyber-foreground shadow-glow" : "bg-surface-2 text-muted-foreground"}`}
                >
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                {unlocked
                  ? "Ready to claim."
                  : c.month === 1
                    ? `Complete Month 1 to unlock (${summary.pct}%)`
                    : "Complete previous months to unlock"}
              </div>
            </div>
          );
        })}
