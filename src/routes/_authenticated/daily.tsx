    ...
    <div className="mx-auto max-w-5xl px-4 py-6 md:px-8">
      <header className="glass-panel relative overflow-hidden rounded-2xl p-6">
        ...
        <div className="relative">
          <div className="flex flex-wrap items-center gap-2">
            <Badge className="bg-warning/20 text-warning"><AlertTriangle className="mr-1 h-3 w-3" /> LIVE TICKET</Badge>
            <Badge className={SEV_COLOR[mission.severity]}>{mission.severity.toUpperCase()}</Badge>
            <Badge variant="outline"><Building2 className="mr-1 h-3 w-3" />{mission.bu}</Badge>
          </div>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <Card className="glass-panel p-5">
          <div className="text-xs font-semibold uppercase tracking-[0.15em] text-cyber">Objectives</div>
        ...
        <Card className="glass-panel p-5">
          <div className="text-xs font-semibold uppercase tracking-[0.15em] text-cyber">Dataset available</div>
          <p className="mt-2 text-sm text-muted-foreground">Referenced dataset: <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-xs">{mission.datasetRef}</code></p>
          <p className="mt-3 text-xs text-muted-foreground">Explore it in the Labs page and copy relevant rows into your findings.</p>
          <div className="mt-4">
            <div className="text-xs font-semibold text-cyber">Skills exercised</div>
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {mission.skills.map((s) => <Badge key={s} variant="outline">{s}</Badge>)}
            </div>
          </div>
          <details className="mt-4">
            <summary className="cursor-pointer text-xs font-semibold text-warning">Reveal hint</summary>
      ...
      <Card className="glass-panel mt-6 p-5">
        <div className="text-xs font-semibold uppercase tracking-[0.15em] text-cyber">Your findings report</div>
        <Textarea
          value={findings}
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="text-xs text-muted-foreground">AI-graded against senior SOC criteria.</span>
          <Button onClick={submit} disabled={busy || !findings.trim()} className="cyber-gradient text-cyber-foreground">
      ...
      {result && (
        <Card className="glass-panel mt-4 border-cyber/40 p-5">
          <div className="flex items-center gap-3">
            ...
            <Badge className="ml-auto cyber-gradient text-cyber-foreground"><CheckCircle2 className="mr-1 h-3 w-3" />+40 XP</Badge>
          </div>
