      ...
      <div className="glass-panel rounded-2xl p-6">
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold tracking-tight">{profile?.display_name ?? "Analyst"}</h1>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {(roles ?? []).map((r) => (
                <Badge key={r} className="cyber-gradient text-cyber-foreground capitalize">{r}</Badge>
              ))}
            </div>
          </div>
        </div>
