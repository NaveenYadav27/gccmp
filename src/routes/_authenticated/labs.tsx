...
const LABS = [
  { icon: Terminal, name: "Linux Investigation", mission: "M1.5", ready: true, tag: "Foundational" },
  { icon: Network, name: "Packet Capture 101", mission: "M1.4", ready: true, tag: "Networking" },
  { icon: Server, name: "Windows Event Log Triage", mission: "M1.6", ready: true, tag: "Windows/AD" },
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 md:px-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Hands-on environments tied to each mission. Enterprise-shaped, not slide examples.</p>
      </header>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {LABS.map((l) => (
          <div key={l.name} className="glass-panel group relative overflow-hidden rounded-2xl p-5 transition-all hover:-translate-y-0.5 hover:border-cyber/50">
            <div className="mb-4 flex items-start justify-between">
              ...
              <Badge variant="outline" className="text-[10px]">{l.mission}</Badge>
            </div>
